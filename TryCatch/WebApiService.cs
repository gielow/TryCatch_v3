...
namespace TryCatch
{
    public class WebApiService
    {
        ...
        {
            return await GetAsync<T>(action, string.Empty);
        }
        ...
        public async Task<TR> PostAsync<T, TR>(string action, T data, string authToken = null)
        {
            using (var client = new HttpClient())
            {
                if (!authToken.IsNullOrWhiteSpace())
                ...
                if (result.IsSuccessStatusCode)
                {
                    return await result.Content.ReadAsAsync<TR>();
                }
